...
(defun slack-print-users-chat-list (element)
  ...
  (let* ((is_user_deleted (cdr (assoc 'is_user_deleted element)))
    ...
    (insert (propertize (format " Created:%s" (print-time created)) 'face '(:foreground "purple")))
    (insert (propertize (format " id:%s" id) 'face '(:foreground "yellow")))
    (insert (propertize (format " Deleted?:%s" is_user_deleted) 'face '(:foreground "green")))
    (princ "\n")))

(defun slack-get-channel-history (name id)
  (lexical-let ((data `(("token" . ,slack-token)))
		(channel-buffer (format "*slack-%s-history*" name))
		(uri (format "https://slack.com/api/channels.history?token=%s&channel=%s&count=%s" slack-token id 200))
...
(defun slack-get-group-history (name id)
  (lexical-let ((data `(("token" . ,slack-token)))
		(channel-buffer (format "*slack-%s-history*" name))
		(uri (format "https://slack.com/api/groups.history?token=%s&channel=%s&count=%s" slack-token id 200))
		(id (slack-get-channel-id-from-name name))
		)
    (web-http-get
     (lambda (httpc header my-data)
...
(defun slack-list-emojis ()
  ...
  (lexical-let ((data `(("token" . ,slack-token)))
		(channel-buffer (format "*slack-%s-history*" name))
		(uri (format "https://slack.com/api/emoji.list?token=%s&channel=%s&count=%s" slack-token id 200))
		(id (slack-get-channel-id-from-name name)))
    (web-http-get
     (lambda (httpc header my-data)
       (with-output-to-temp-buffer channel-buffer
...
(defun slack-post-message (channel message)
  (interactive "sSlack Channel:\nsMessage: ")
  (lexical-let* ((data `(("token" . ,slack-token)))
		 (channel-buffer "*slack-post*")
 		 (uri (format "https://slack.com/api/chat.postMessage?token=%s&channel=%%23%s&text=%s&username=%s" slack-token channel (replace-regexp-in-string " " "%20" message) slack-username)))
...
(defun slack-print-search-results (element)
  (let ((text (cdr (assoc 'text element)))
    ...
    (slack-create-user-link-in-buffer "green" username user)
    ...
    (insert (propertize (format " %s" text) 'face '(:foreground "green")))
    (princ "\n")))
...
(defun slack-print-channel-history (element)
  (let* ((ts (cdr (assoc 'ts element)))
	 ...
	 (user (cdr (assoc 'user element)))
	 (type (cdr (assoc 'type element))))
    (insert (propertize (format " %s" (slack-get-username-from-id user)) 'face '(:foreground "green")))
...
(defun slack-print-list-channels (element)
  (let* ((num_members (format "%s" (cdr (assoc 'num_members element))))
    ...
    (insert (propertize (format " #%s" num_members) 'face '(:foreground "green")))
    ;;(insert (propertize (format " %s" is_member) 'face '(:foreground "red")))
    ;;(insert (propertize (format " %s" is_archived) 'face '(:foreground "blue")))
    (insert (propertize (format " creator: %s" (slack-get-username-from-id creator)) 'face '(:foreground "green")))
...
(defun slack-print-list-users (element)
  (let*
      ((has_files (format "%s" (cdr (assoc 'has_files element))))
       (is_ultra_restricted (format "%s" (cdr (assoc 'is_ultra_restricted element))))
       (is_restricted (format "%s" (cdr (assoc 'is_restricted element))))
       (is_primary_owner (format "%s" (cdr (assoc 'is_primary_owner element))))
       (is_owner (format "%s" (cdr (assoc 'is_owner element))))
       (is_admin (format "%s" (cdr (assoc 'is_admin element))))
       (profile (format "%s" (cdr (assoc 'profile element))))
       ;;(email (cdr (assoc 'email profile)))
       (email (symbol-name (cdr (read (format "%s)" (elt (split-string (substring profile (string-match "(email .*)" profile)) "\)") 0))))))
       ;;(real_name (symbol-name (cdr (read (format "%s)" (elt (split-string (substring profile (string-match "(real_name .)" profile)) "\)") 0))))))
       ;; (image_original (format "%s" (cdr (assoc 'image_original profile))))
       ;; (image_192 (format "%s" (cdr (assoc 'image_192 profile))))
       ;; (image_72 (format "%s" (cdr (assoc 'image_72 profile))))
       ;; (image_48 (format "%s" (cdr (assoc 'image_48 profile))))
       ;; (image_32 (format "%s" (cdr (assoc 'image_32 profile))))
       ;; (image_24 (format "%s" (cdr (assoc 'image_24 profile))))
       ;; (last_name (format "%s" (cdr (assoc 'last_name profile))))
       ;; (first_name (format "%s" (cdr (assoc 'first_name profile))))
       (tz_offset (format "%s" (cdr (assoc 'tz_offset element))))
       (tz (format "%s" (cdr (assoc 'tz element))))
       (phone (format "%s" (cdr (assoc 'phone element))))
       (skype (format "%s" (cdr (assoc 'skype element))))
       (real_name (format "%s" (cdr (assoc 'real_name element))))
       (color (format "%s" (cdr (assoc 'color element))))
       (status (format "%s" (cdr (assoc 'status element))))
       (deleted (format "%s" (cdr (assoc 'deleted element))))
       (name (format "%s" (cdr (assoc 'name element))))
       (id (format "%s" (cdr (assoc 'id element))))
       (short-name (format "%s" (nth 0  (split-string email "@")))))
    ;;(if real_name
    ;;(puthash id real_name slack-users)
    (puthash id short-name slack-users)
    ;;)
    ;;(insert (propertize (format " profile: %s" profile) 'face '(:foreground "green")))
    ...
    ;;    (insert (propertize (format " email:%s" email) 'face '(:foreground "green")))
    (insert (propertize (format " is_ultra_restricted:%s" is_ultra_restricted) 'face '(:foreground "purple")))
    (insert (propertize (format " is_restricted:%s" is_restricted) 'face '(:foreground "green")))
    (insert (propertize (format " is_admin:%s" is_restricted) 'face '(:foreground "green")))
    (insert (propertize (format " phone:%s" phone) 'face '(:foreground "green")))
    (insert (propertize (format " skype:%s" skype) 'face '(:foreground "green")))
    (princ "\n")))
